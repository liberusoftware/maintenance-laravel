...
final class CreateAssetSpecification
{
    public function handle(int $teamId, Asset $asset, array $attributes): AssetSpecification
    {
        ...
        if ($key === '' || trim((string) ($attributes['value'] ?? '')) === '') {
            throw ValidationException::withMessages(['key' => 'A specification key and value are required.']);
        }
        if (AssetSpecification::query()->where('asset_id', $asset->getKey())->where('key', $key)->exists()) {
            throw ValidationException::withMessages(['key' => 'That specification already exists for the asset.']);
        }
