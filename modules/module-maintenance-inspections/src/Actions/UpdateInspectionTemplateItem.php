...
final class UpdateInspectionTemplateItem
{
    public function handle(int $teamId, InspectionTemplate $template, string $key, array $changes): InspectionTemplate
    {
        ...
        if (! in_array($definition['type'] ?? 'string', ['numeric', 'number', 'boolean', 'bool', 'choice', 'select', 'string', 'text'], true)) throw ValidationException::withMessages(['type' => 'The checklist item type is not supported.']);
        if (in_array($definition['type'] ?? null, ['choice', 'select'], true) && ! is_array($definition['options'] ?? null)) throw ValidationException::withMessages(['options' => 'Choice checklist items require options.']);
