...
final class DispatchScheduleEntry
{
    public function handle(int $teamId, ScheduleEntry $entry, int $userId, ?int $actorId = null, ?string $notes = null): Dispatch
    {
        ...
        if ($userId < 1) {
            throw ValidationException::withMessages(['user_id' => 'An engineer is required.']);
        }
        if (Dispatch::query()->where('schedule_entry_id', $entry->getKey())->where('user_id', $userId)->whereIn('status', ['offered', 'accepted'])->exists()) {
            throw ValidationException::withMessages(['user_id' => 'This engineer has already been dispatched to the schedule entry.']);
        }
