...
final class MaintenanceNoteController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->filled('noteable_type')) {
            $query->where('noteable_type', $request->string('noteable_type')->toString());
        }
        if ($request->filled('noteable_id')) {
            $query->where('noteable_id', $request->integer('noteable_id'));
        }
    ...
    private function note(Request $request, string $key): MaintenanceNote
    {
        return MaintenanceNote::query()->where('team_id', $this->teamId($request))->findOrFail($key);
    }

    private function resource(MaintenanceNote $note): array
    {
        return ['id' => (string) $note->getKey(), 'type' => 'maintenance-note', 'attributes' => ['content' => $note->content, 'noteable_type' => $note->noteable_type, 'noteable_id' => $note->noteable_id, 'created_by' => $note->created_by, 'updated_by' => $note->updated_by, 'created_at' => $note->created_at?->toISOString(), 'updated_at' => $note->updated_at?->toISOString()]];
    }
}
