...
- `publish-maintenance-modules` synchronizes `modules/module-maintenance-*` to their matching `liberusoftware` repositories, opens pull requests, and accepts `--merge` when the package changes are verified. It excludes dependency and build artifacts; pass package names to limit the sweep.
